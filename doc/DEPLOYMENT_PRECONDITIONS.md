...
3. **One feature at a time, and only after its audit scope clears.** Never
   ...
   it is the baseline the other four are measured against. `DEPLOYMENT_CTV` is
   arity-neutral and could in principle follow later; it is held to the same
   flag-day so that the rule has no exception to remember, and the test below
   enforces that. Either mechanism gives a loosening measured against the
   intermediate release, and it splits every
