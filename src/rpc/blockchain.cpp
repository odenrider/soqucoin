...
static UniValue getblockstats(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "or minted per asset by consensus and are never a fee. The other amount statistics\n"
            "(total_out, minoutamount, maxoutamount, dustouts) sum every output regardless of asset.\n"
