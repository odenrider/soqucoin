...
//
// usdsoq_supply_and_bootstrap_reject_path_tests.cpp — reject-path coverage for
// two USDSOQ invariant guards:
//
//   bad-usdsoq-supply-overflow      the supply guard in ConnectBlock's mint path
//   bad-usdsoq-bootstrap-reentry    the re-entry guard in ConnectBlock's
//                                   USDSOQ bootstrap branch
//
// Both are the USDSOQ twins of rules covered in
// btcsoq_lifecycle_harness_tests.cpp. Each fires only on an authority or
// accounting error or on a damaged local state, so each case injects that state
// and proves the guard fires:
//
//   supply-overflow — CUSDSOQSupply::Mint fails when total_minted leaves
//     MoneyRange. Reached here by preloading the counter.
//
//   bootstrap-reentry — the database holds an authority outpoint while the
//     in-memory global is null, which a partial reindex or a corrupted datadir
//     produces. Reached here by nulling the global and leaving the database
//     alone.
//
// Each case carries a presence control, because a reject asserted against an
// empty database or an untouched counter would prove nothing.
...
struct UsdsoqSupplyBootstrapSetup : public DilithiumChainSetup {
    ...
    CMutableTransaction BuildBootstrap(const CTransaction& feeCoinbase, CAmount mintV7)
    {
        ...
        // auth.Sign builds its whole witness stack.
